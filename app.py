...
st.markdown(
    ...
    /* Degradado de fondo para el sidebar */
    section[data-testid="stSidebar"] {
        background-image: linear-gradient(180deg, #091a0f 0%, #030805 100%) !important;
    ...
    section[data-testid="stSidebar"] div {
        color: #e2f9e6 !important;
    }
    
    /* Botón Primario (Llamativo, Gradiente Verde Forestal) */
    ...
    div.stButton > button[kind="primary"]:hover {
        ...
        background: linear-gradient(135deg, #2eff7d 0%, #2ecc71 100%) !important;
    }
    ...
    /* Bordes redondeados en file uploaders */
...
st.markdown(
    ...
    <div style="background: linear-gradient(135deg, #132e18 0%, #08170d 100%); 
        ...
        <h1 style="color: white; margin: 0; font-size: 30px; font-weight: 700; display: flex; align-items: center; font-family: 'Outfit', sans-serif;">
            <span style="margin-right: 15px;">🌿</span> Monitor IA — Deforestación e Incendios
        </h1>
        <p style="color: #a3e2b2; margin: 8px 0 0 0; font-size: 14.5px; font-family: 'Outfit', sans-serif; opacity: 0.9; line-height: 1.5;">
            Plataforma avanzada de monitoreo satelital con imágenes <b>Sentinel-2</b> (Copernicus). 
            Aplica inteligencia artificial y análisis multiespectral para detectar de forma automatizada talas masivas de bosques, incendios y perturbaciones de vegetación.
        </p>
    </div>
    """,
    unsafe_allow_html=True
...
if not archivos_listos:
    st.markdown(
        ...
        <div style="background-color: #f7fafc; padding: 30px; border-radius: 12px; border: 1px solid #e2e8f0; margin-top: 10px;">
            <h3 style="color: #2d3748; margin-top: 0; font-family: 'Outfit', sans-serif; font-weight: 600;">👋 ¡Bienvenido al Sistema de Monitoreo!</h3>
            <p style="color: #4a5568; font-size: 15px; margin-bottom: 20px; line-height: 1.6;">
                Esta herramienta analiza la pérdida de cobertura forestal (deforestación, incendios o talas) comparando imágenes satelitales Sentinel-2 en dos períodos de tiempo. 
                Para comenzar, selecciona una de las siguientes opciones en el panel izquierdo:
            </p>
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap: 15px; margin-bottom: 25px;">
                <div style="background: white; padding: 20px; border-radius: 8px; border: 1px solid #cbd5e0; border-top: 4px solid #2ecc71; box-shadow: 0 4px 6px rgba(0,0,0,0.02);">
                    ...
                    <p style="margin: 0; color: #718096; font-size: 13.5px; line-height: 1.5;">
                        Carga instantáneamente imágenes locales del Maule (Chile) y ejecuta el análisis con un solo clic. Ideal para probar las capacidades.
                    </p>
                </div>
                <div style="background: white; padding: 20px; border-radius: 8px; border: 1px solid #cbd5e0; border-top: 4px solid #3498db; box-shadow: 0 4px 6px rgba(0,0,0,0.02);">
                    <h4 style="margin: 0 0 10px 0; color: #1f3a60; font-weight: bold;">📂 2. Subir Archivos Propios</h4>
                    <p style="margin: 0; color: #718096; font-size: 13.5px; line-height: 1.5;">
                        Sube las bandas multiespectrales B04 (Rojo) y B08 (NIR) en formato TIFF descargadas de Copernicus Browser.
                    </p>
                </div>
                <div style="background: white; padding: 20px; border-radius: 8px; border: 1px solid #cbd5e0; border-top: 4px solid #9b59b6; box-shadow: 0 4px 6px rgba(0,0,0,0.02);">
                    <h4 style="margin: 0 0 10px 0; color: #4a1f60; font-weight: bold;">🛰️ 3. Descarga Automática</h4>
                    <p style="margin: 0; color: #718096; font-size: 13.5px; line-height: 1.5;">
                        Ingresa tus credenciales de Copernicus Data Space para buscar, descargar y procesar automáticamente imágenes del área de interés.
                    </p>
                </div>
            </div>
        </div>
    ...
    if analizar or ("zonas" not in st.session_state):
        with st.spinner("Procesando imágenes multiespectrales y ejecutando algoritmos..."):
            ...
            zonas, mascara_alerta = detectar_zonas(
                ...
                nbr1=nbr1 if 'nbr1' in dir() else None,
                nbr2=nbr2 if 'nbr2' in dir() else None,
            )
